// VehicleDetailTCrossBitono.tsx
// Vista de detalle para Volkswagen T-Cross Bitono (2025)
...
const HiluxImages = [
  ...
  "/FerradasAutomotores/ToyotaHiluxDX/d(2).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(3).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(4).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(5).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(6).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(7).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(8).jpg",
  "/FerradasAutomotores/ToyotaHiluxDX/d(9).jpg",
];

const HiluxSpecs = [
  { label: "Año", value: "2025" },
  { label: "Motor", value: "2.4" },
  { label: "Transmisión", value: "Automática" },
  { label: "Potencia", value: "150 CV" },
...
const VehicleDetailToyotaHiluxDx: React.FC = () => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const [current, setCurrent] = useState(0);
  const navigate = useNavigate();
  const prev = () => setCurrent((c) => (c === 0 ? HiluxImages.length - 1 : c - 1));
  const next = () => setCurrent((c) => (c === HiluxImages.length - 1 ? 0 : c + 1));
  return (
    <section className="relative min-h-screen w-full bg-white flex flex-col justify-center items-center overflow-x-hidden">
      ...
      <div className="hidden lg:block fixed inset-0 z-0">
        <img
          src={HiluxImages[current]}
          alt="Volkswagen T-Cross Bitono"
      ...
      <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center mx-auto mt-20 bg-gray-100 rounded-2xl shadow-lg z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl lg:mt-32">
        ...
        <img src={HiluxImages[current]} alt="Volkswagen T-Cross Bitono" className="object-contain h-full w-full rounded-2xl" loading="lazy" />
        ...
        <div className="absolute left-8 bottom-8 z-30 bg-white/80 px-4 py-2 rounded-xl shadow">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 drop-shadow-none">Toyota <span className="text-red-600">Hilux DX</span></h1>
        </div>
      </div>
...
export default VehicleDetailToyotaHiluxDx;
