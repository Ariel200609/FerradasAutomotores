// VehicleDetailAmarokBlackStyleG2.tsx
// Vista de detalle para Volkswagen Amarok Black Style G2 (2025)
...
const VehicleDetailAmarokBlackStyleG2: React.FC = () => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const [current, setCurrent] = useState(0);
  const navigate = useNavigate();
  const prev = () => setCurrent((c) => (c === 0 ? amarokBlackStyleG2Images.length - 1 : c - 1));
  const next = () => setCurrent((c) => (c === amarokBlackStyleG2Images.length - 1 ? 0 : c + 1));
  return (
    <section className="relative min-h-screen w-full bg-white flex flex-col justify-center items-center overflow-x-hidden">
      ...
      <div className="hidden lg:block fixed inset-0 z-0">
        <img
          src={amarokBlackStyleG2Images[current]}
          alt="Volkswagen Amarok Black Style G2"
      ...
      <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center mx-auto mt-20 bg-gray-100 rounded-2xl shadow-lg z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl lg:mt-32">
        ...
        <img src={amarokBlackStyleG2Images[current]} alt="Volkswagen Amarok Black Style G2" className="object-contain h-full w-full rounded-2xl" loading="lazy" />
        ...
        <div className="absolute left-8 bottom-8 z-30 bg-white/80 px-4 py-2 rounded-xl shadow">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 drop-shadow-none">Volkswagen <span className="text-red-600">Amarok Black Style G2</span></h1>
        </div>
      </div>
      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl mt-8 p-8 flex flex-col md:flex-row justify-between items-center gap-8 shadow-lg border border-gray-100 z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
          {amarokBlackStyleG2Specs.map((spec) => (
...
export default VehicleDetailAmarokBlackStyleG2; 