// VehicleDetailRaptor.tsx
// Vista de detalle para Ford Ranger Raptor (2024)
...
const raptorSpecs = [
  ...
  { label: "Motor", value: "3.0 V6 Nafta" },
  { label: "Transmisión", value: "Automática" },
  { label: "Potencia", value: "400 CV" },
  { label: "Color", value: "Gris Stone" },
  { label: "Puertas", value: "4" },
  { label: "Tracción", value: "4x4" },
  { label: "Kilometraje", value: "25.000 km" },
];

const VehicleDetailRaptor: React.FC = () => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const [current, setCurrent] = useState(0);
  const navigate = useNavigate();
  const prev = () => setCurrent((c) => (c === 0 ? raptorImages.length - 1 : c - 1));
  ...
  return (
    <section className="relative min-h-screen w-full bg-white flex flex-col justify-center items-center overflow-x-hidden">
      ...
      <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center mx-auto mt-20 bg-gray-100 rounded-2xl shadow-lg z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl lg:mt-32">
        ...
        <img src={raptorImages[current]} alt="Ford Ranger Raptor" className="object-contain h-full w-full rounded-2xl" loading="lazy" />
        ...
        <div className="absolute left-8 bottom-8 z-30 bg-white/80 px-4 py-2 rounded-xl shadow">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 drop-shadow-none">Ford <span className="text-red-600">Ranger Raptor</span></h1>
        </div>
      </div>
...
export default VehicleDetailRaptor; 