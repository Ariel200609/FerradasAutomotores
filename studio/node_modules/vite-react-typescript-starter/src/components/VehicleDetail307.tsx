// VehicleDetail307.tsx
// Vista de detalle para Peugeot 307 XS Cuero (2011)
...
const specs307 = [
  { label: "Año", value: "2011" },
  { label: "Motor", value: "2.0 Nafta" },
  { label: "Transmisión", value: "Manual" },
  { label: "Color", value: "Champagne" },
  { label: "Puertas", value: "5" },
  { label: "Tracción", value: "Manual" },
  { label: "Kilometraje", value: "125.000 km" },
];

const VehicleDetail307: React.FC = () => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const [current, setCurrent] = useState(0);
  const navigate = useNavigate();
  const prev = () => setCurrent((c) => (c === 0 ? images307.length - 1 : c - 1));
  const next = () => setCurrent((c) => (c === images307.length - 1 ? 0 : c + 1));
  return (
    <section className="relative min-h-screen w-full bg-white flex flex-col justify-center items-center overflow-x-hidden">
      ...
      <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center mx-auto mt-20 bg-gray-100 rounded-2xl shadow-lg z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl lg:mt-32">
        ...
        <img src={images307[current]} alt="Peugeot 307" className="object-contain h-full w-full rounded-2xl" loading="lazy" />
        ...
        <div className="absolute left-8 bottom-8 z-30 bg-white/80 px-4 py-2 rounded-xl shadow">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 drop-shadow-none">Peugeot <span className="text-red-600">307</span></h1>
        </div>
      </div>
...
export default VehicleDetail307; 