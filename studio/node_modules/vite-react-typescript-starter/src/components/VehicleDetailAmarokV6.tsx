// VehicleDetailAmarokV6.tsx
// Vista de detalle para Volkswagen Amarok V6 (2022)
...
const amarokV6Specs = [
  ...
  { label: "Color", value: "Gris industrial" },
  { label: "Puertas", value: "4" },
  { label: "Tracción", value: "4x4" },
  { label: "Kilometraje", value: "90.000 km" },
];

const VehicleDetailAmarokV6: React.FC = () => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const [current, setCurrent] = useState(0);
  const navigate = useNavigate();
  const prev = () => setCurrent((c) => (c === 0 ? amarokV6Images.length - 1 : c - 1));
  const next = () => setCurrent((c) => (c === amarokV6Images.length - 1 ? 0 : c + 1));
  return (
    <section className="relative min-h-screen w-full bg-white flex flex-col justify-center items-center overflow-x-hidden">
      ...
      <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center mx-auto mt-20 bg-gray-100 rounded-2xl shadow-lg z-10 lg:bg-white/80 lg:backdrop-blur-md lg:shadow-2xl lg:mt-32">
        ...
        <img src={amarokV6Images[current]} alt="Volkswagen Amarok V6" className="object-contain h-full w-full rounded-2xl" loading="lazy" />
        ...
        <div className="absolute left-8 bottom-8 z-30 bg-white/80 px-4 py-2 rounded-xl shadow">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 drop-shadow-none">Volkswagen <span className="text-red-600">Amarok V6</span></h1>
        </div>
      </div>
...
export default VehicleDetailAmarokV6; 